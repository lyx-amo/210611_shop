        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{active:getOrder[0] === '1'}" @click="updateOrder('1')">
                  <a href="javascript:;">
                    综合
                    <i v-if="getOrder[0] === '1'" class="iconfont" :class="getOrder[1]==='desc' ? 'icon-down' : 'icon-up'"></i>
                  </a>
                </li>
                <li>
                ...
                <li :class="{active:getOrder[0] === '2'}" @click="updateOrder('2')">
                  <a href="javascript:;" >
                    价格
                    <i v-if="getOrder[0] === '2'" class="iconfont" :class="getOrder[1]==='desc' ? 'icon-down' : 'icon-up'"></i>
                  </a>
                </li>
              </ul>
            </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      updateOrder(orderFlag){
          // 更新options中的order
          // 点击的是当前排序项: 只需要切换orderType
          // 点击的不是当前排序项:  更新orderFlag为指定的值, orderType更新为desc
          // 请求获取商品分页列表
        let [flag,type] =  this.getOrder
        //判断flag是否相同,相同则更新type
        if(orderFlag === flag){
      ...
      setTrademark(trademark){
        ...
        //如果没有,则添加进options,并更新商品列表
        // this.options.trademark = trademark
        this.$set(this.options,'trademark',trademark)//实现响应式数据,能快速更改页面,而不需要等待网络请求再修改页面
        this.getShopList()
      },
      removeKeyword(){
      ...
      updateParams(){
        const {keyword} = this.$route.params //拿到输入的keyword
